...
from model import M_Unet, init_weights
from metrices1 import IoU,Fscore,Accuracy,Sensitivity,Specificity
###### data folder create #####
local_path = "./datasets/data_folder/"
...
def main():
    ...
    for e in range(epochs):
        ...
        file_name = os.path.join(ckp_path+'task_3_weights_epoch_{}'.format(e) + '.pt')
    ...
    (pd.DataFrame.from_dict(data=loss_history, orient='columns').to_csv('./logfiles/loss.csv', header=['epoch','train_loss','val_loss']))
    (pd.DataFrame.from_dict(data=train_metrices_history, orient='columns').to_csv('./logfiles/train_metrices.csv', header=["epoch","acc","iou","dice","sens","spec"]))
    (pd.DataFrame.from_dict(data=val_metrices_history, orient='columns').to_csv('./logfiles/val_metrices.csv', header=["epoch","acc","iou","dice","sens","spec"]))
def val():
    model.eval()
